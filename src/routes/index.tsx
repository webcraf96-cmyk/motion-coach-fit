...
function ProgressScreen({ workouts, profile, onStart }: { workouts: Workout[]; profile: Profile; onStart: (name: string) => void }) {
  ...
  const selected = period === "all" ? workouts : workouts.filter(w => new Date(w.created_at) >= start);
  const days = Array.from({ length: 7 }, (_, i) => { const date = new Date(now); date.setDate(now.getDate() - 6 + i); return date; });
  const minutes = days.map(day => workouts.filter(w => new Date(w.created_at).toDateString() === day.toDateString()).reduce((sum, w) => sum + w.duration_seconds / 60, 0));
  const maxMinutes = Math.max(1, ...minutes);
  const totalMinutes = selected.reduce((sum, w) => sum + w.duration_seconds, 0);
  return <div className="screen-enter"><div className="page-heading"><span className="eyebrow">YOUR WORK, IN NUMBERS</span><h1>Progress that<br/><em>adds up.</em></h1><p>A clearer view of how far you’ve come.</p></div>
    <div className="period-control" aria-label="Progress period">{([ ["week", "This week"], ["month", "This month"], ["all", "All time"] ] as const).map(([key, label]) => <Button key={key} variant="ghost" className={period === key ? "period-active" : ""} onClick={() => setPeriod(key)} aria-pressed={period === key}>{label}</Button>)}</div>
    <div className="dashboard-grid progress-metrics"><MetricTile label="TOTAL REPS" value={selected.reduce((sum, w) => sum + w.reps, 0).toLocaleString()} note="across all movements"/><MetricTile label="WORKOUTS" value={`${selected.length}`} note="sessions saved"/><MetricTile label="CALORIES" value={selected.reduce((sum, w) => sum + w.calories, 0).toLocaleString()} note="estimated kcal"/><MetricTile label="STREAK" value={`${profile.streak} days`} note="keep the rhythm"/></div>
    <ExerciseRecords workouts={workouts} onStart={onStart} />
    <section className="content-section chart-section"><div className="section-heading"><div><span className="eyebrow">LAST 7 DAYS</span><p className="subline">Workout minutes by day</p></div><span className="chart-total">{Math.round(totalMinutes / 60)} min</span></div><div className="tall-chart">{minutes.map((v, i) => <div className="tall-day" key={i}><span className={`tall-bar ${i === 6 ? "today" : ""}`} style={{ height: `${Math.max(7, v / maxMinutes * 100)}%` }} title={`${Math.round(v)} minutes`}/><span>{days[i].toLocaleDateString(undefined, { weekday: "narrow" })}</span></div>)}</div></section>
    <section className="coach-card coach-wide"><span className="coach-icon"><TrendingUp size={18}/></span><div className="coach-copy"><span className="eyebrow">TRAINING SNAPSHOT</span><p>{selected.length ? `You logged ${selected.reduce((sum, w) => sum + w.reps, 0).toLocaleString()} reps across ${selected.length} sessions in this period.` : "Your first workout will start filling in your progress."}</p></div></section>
    <div className="section-heading list-heading"><span className="eyebrow">RECENT WORKOUTS</span><span className="mini-caption">{selected.length} SAVED</span></div>{selected.length ? <div className="exercise-list">{selected.slice(0, 8).map(w => <HistoryItem key={w.id} workout={w}/>)}</div> : <div className="empty-state"><History size={24}/><p>Your next session starts a new record.</p><Button className="mini-start" onClick={() => onStart("Squats")}>Start workout</Button></div>}
  </div>;
}
