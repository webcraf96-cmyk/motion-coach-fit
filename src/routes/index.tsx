...
import {
  ...
  Volume2, VolumeX, Weight, X, Camera, CameraOff, Eye, EyeOff, LogOut,
} from "lucide-react";
...
export const Route = createFileRoute("/")({
  head: () => ({ meta: [
    { title: "REPVERSE — Your camera. Your workout. Your progress." },
    { name: "description", content: "Train with REPVERSE: camera-guided workouts, personal progress, and a little friendly competition." },
    { property: "og:title", content: "REPVERSE — Your camera. Your workout. Your progress." },
  ...
  ] }), component: RepverseApp,
});

function RepverseApp() {
  ...
  useEffect(() => {
    ...
    supabase.auth.getSession().then(async ({ data }) => {
      ...
      const { data: history } = await supabase.from("fitness_workouts").select("id,exercise,reps,duration_seconds,calories,form_score,xp_earned,created_at,mode").eq("user_id", current.id).order("created_at", { ascending: false }).limit(30);
      if (active && history) setWorkouts(history as Workout[]);
    });
  ...
  const finishWorkout = async () => {
    ...
    if (user) {
      ...
      try { localStorage.setItem("repverse-demo-workouts", JSON.stringify([result, ...JSON.parse(localStorage.getItem("repverse-demo-workouts") || "[]")].slice(0, 30))); } catch { /* demo remains usable when storage is full */ }
    }
  };
    ...
    {authView ? <AuthScreen view={authView} setView={setAuthView} onDemo={() => { setAuthView(null); setIsDemo(true); toast.success("Demo mode ready", { description: "Your workouts are saved on this device." }); }} email={authEmail} setEmail={setAuthEmail} password={authPassword} setPassword={setAuthPassword} name={fullName} setName={setFullName} showPassword={showPassword} setShowPassword={setShowPassword} busy={authBusy} onSubmit={signIn} onSocial={googleLogin} goal={selectedGoal} setGoal={setSelectedGoal} onOnboarding={saveOnboarding} /> : <>
      {!workoutOpen && !done && <>
        <header className="topbar">
          <div className="brand-lockup"><span className="brand-mark">R</span><div><p className="brand-name">REPVERSE</p><p className="brand-sub">TRAINING CONSOLE</p></div></div>
        ...
        <div className="page-content" key={tab}>
          ...
          {tab === "Workout" && <WorkoutLibrary onStart={startWorkout} onHistory={() => setTab("Progress")} />}
          {tab === "Progress" && <ProgressScreen workouts={workouts} profile={profile} onStart={() => startWorkout("Squats")} />}
...
function HomeScreen({ name, profile, exercises: list, challenge, onStart, onTab, workouts }: { name: string; profile: Profile; exercises: Exercise[]; challenge: number; onStart: (n: string) => void; onTab: (t: Tab) => void; workouts: Workout[] }) {
  ...
  return <div className="screen-enter"><div className="greeting-row"><div><p className="greeting">Good morning,</p><h1>{name} <span>✦</span></h1></div><button className="notification-button" aria-label="Notifications" onClick={() => toast.message("You’re all caught up.")}><Bell size={19}/><i/></button></div>
    ...
    <div className="dashboard-grid metric-grid"><MetricTile label="TOTAL REPS" value={workouts.reduce((a, w) => a + w.reps, 2486).toLocaleString()} note="+18% this week"/><MetricTile label="CALORIES" value="4,230" note="kcal burned"/></div>
    ...
    <section className="content-section coach-card"><span className="coach-icon"><Sparkles size={18}/></span><div className="coach-copy"><span className="eyebrow">REPVERSE COACH</span><p>You’re building a strong rhythm. Your squat consistency is up <b>18%</b> this week.</p></div><ChevronRight size={17}/></section>
  </div>;
}
